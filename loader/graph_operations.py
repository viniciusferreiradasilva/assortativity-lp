...
def delete_self_edges(graph):
    graph.delete_edges([edge for edge in graph.es if (edge.source == edge.target)])
    return graph


# Method that removes the edges in the probe list from the graph. The edge list must be a dictionary which
# the key is a edge and the value is the weight.
def delete_edges(graph, edges_probe_list):
    graph.delete_edges(edges_probe_list.keys())


# Method that re-adds the edges in the probe list to the graph. The edge list must be a dictionary which
# the key is a edge and the value is the weight.
def readd_edges(graph, edges_probe_list):
...
def create_random_edges_probe_list(graph, proportion):
    edges_probe_list = {}
...
def create_k_edges_probe_list(graph, edgelist, i, k):
    edges_probe_list = {}
